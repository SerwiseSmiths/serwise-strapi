/**
 * Migrates subscriptions (+ their device-type & part relations) and the
 * app-version single type from prod Strapi → local Strapi.
 *
 * Run with:  npm run migrate-to-local
 *
 * NOTE: APK files on app-version are NOT migrated (binary upload not
 *       supported via REST). Re-upload them manually in the local admin.
 */

export {};

const LOCAL = 'http://localhost:1337';
const PROD = 'https://serwise-strapi.onrender.com';

const LOCAL_TOKEN = '';
const PROD_TOKEN = '';

// SOURCE = prod, DEST = local
const SRC = PROD;
const SRC_TOKEN = PROD_TOKEN;
const DST = LOCAL;
const DST_TOKEN = LOCAL_TOKEN;

// ─── helpers ────────────────────────────────────────────────────────────────

async function get(base: string, token: string, path: string) {
  const res = await fetch(`${base}${path}`, {
    headers: { Authorization: `Bearer ${token}` },
  });
  if (!res.ok) throw new Error(`GET ${base}${path} → ${res.status}: ${await res.text()}`);
  return (await res.json() as any);
}

async function post(base: string, token: string, path: string, body: any) {
  const res = await fetch(`${base}${path}`, {
    method: 'POST',
    headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
    body: JSON.stringify({ data: body }),
  });
  if (!res.ok) throw new Error(`POST ${base}${path} → ${res.status}: ${await res.text()}`);
  return (await res.json() as any).data;
}

async function put(base: string, token: string, path: string, body: any) {
  const res = await fetch(`${base}${path}`, {
    method: 'PUT',
    headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
    body: JSON.stringify({ data: body }),
  });
  if (!res.ok) throw new Error(`PUT ${base}${path} → ${res.status}: ${await res.text()}`);
  return (await res.json() as any).data;
}

async function getAll(base: string, token: string, collection: string, populate = '') {
  const qs = populate ? `&populate=${populate}` : '';
  const r = await get(base, token, `/api/${collection}?pagination[limit]=1000${qs}`);
  return r.data as any[];
}

// ─── step 1: migrate parts ───────────────────────────────────────────────────

async function migrateParts(): Promise<Map<number, number>> {
  console.log('\n📦 Migrating parts...');
  const srcParts = await getAll(SRC, SRC_TOKEN, 'parts');
  const dstParts = await getAll(DST, DST_TOKEN, 'parts');

  const dstIndex = new Map<string, any>();
  for (const p of dstParts) {
    dstIndex.set(`${p.name}|${p.category}|${p.type}`, p);
  }

  const idMap = new Map<number, number>(); // srcId → dstId

  for (const sp of srcParts) {
    const key = `${sp.name}|${sp.category}|${sp.type}`;
    let dstPart = dstIndex.get(key);

    if (!dstPart) {
      dstPart = await post(DST, DST_TOKEN, '/api/parts', {
        name:        sp.name,
        category:    sp.category,
        type:        sp.type,
        status:      sp.status,
        description: sp.description,
      });
      console.log(`  ✓ Created part: ${sp.name}`);
    } else {
      console.log(`  — Part already exists: ${sp.name}`);
    }

    idMap.set(sp.id, dstPart.id);
  }

  console.log(`  Parts done: ${idMap.size} mapped`);
  return idMap;
}

// ─── step 2: build prod → local device type ID map (read-only, no create) ───

async function buildDeviceTypeMap(): Promise<Map<number, number>> {
  console.log('\n📱 Mapping device types (local only — no changes made)...');
  const srcDTs = await getAll(SRC, SRC_TOKEN, 'device-types');
  const dstDTs = await getAll(DST, DST_TOKEN, 'device-types');

  const dstIndex = new Map<string, any>();
  for (const dt of dstDTs) dstIndex.set(dt.type, dt);

  const idMap = new Map<number, number>();
  for (const sdt of srcDTs) {
    const dstDT = dstIndex.get(sdt.type);
    if (dstDT) {
      idMap.set(sdt.id, dstDT.id);
      console.log(`  — Mapped: ${sdt.type}`);
    } else {
      console.log(`  ⚠️  Device type not found on local (skipping): ${sdt.type}`);
    }
  }
  return idMap;
}

// ─── step 3: migrate subscriptions ──────────────────────────────────────────

async function migrateSubscriptions(partMap: Map<number, number>) {
  console.log('\n💳 Migrating subscriptions...');

  const dtMap = await buildDeviceTypeMap();

  const srcSubs = await getAll(SRC, SRC_TOKEN, 'subscriptions', '*');
  const dstSubs = await getAll(DST, DST_TOKEN, 'subscriptions');

  const dstNames = new Set(dstSubs.map((s: any) => s.name));

  for (const ss of srcSubs) {
    if (dstNames.has(ss.name)) {
      console.log(`  — Subscription already exists: ${ss.name}`);
      continue;
    }

    const deviceTypeIds = (ss.device_types ?? [])
      .map((dt: any) => dtMap.get(dt.id))
      .filter(Boolean);

    const serviceMapping = (ss.serviceMapping ?? []).map((sm: any) => ({
      usageIndex:  sm.usageIndex,
      providerCut: sm.providerCut,
      parts: (sm.parts ?? [])
        .map((p: any) => partMap.get(p.id))
        .filter(Boolean),
    }));

    await post(DST, DST_TOKEN, '/api/subscriptions', {
      name:             ss.name,
      plan_type:        ss.plan_type,
      totalServices:    ss.totalServices,
      cost:             ss.cost,
      sub_sales:        ss.sub_sales,
      non_sub_sales:    ss.non_sub_sales,
      sub_profit:       ss.sub_profit,
      non_sub_profit:   ss.non_sub_profit,
      maxDiscount:      ss.maxDiscount,
      lockInPeriod:     ss.lockInPeriod,
      validityDuration: ss.validityDuration,
      description:      ss.description,
      device_types:     deviceTypeIds,
      serviceMapping,
    });

    console.log(`  ✓ Created subscription: ${ss.name}`);
  }
}

// ─── step 4: migrate app-version (text fields only) ─────────────────────────

async function migrateAppVersion() {
  console.log('\n🔢 Migrating app-version...');

  const src = await get(SRC, SRC_TOKEN, '/api/app-version');
  const d = src.data;

  if (!d) {
    console.log('  ⚠️  No app-version data found on prod. Skipping.');
    return;
  }

  await put(DST, DST_TOKEN, '/api/app-version', {
    serwise_version:              d.serwise_version,
    serwise_min_required_version: d.serwise_min_required_version,
    radix_version:                d.radix_version,
    radix_min_required_version:   d.radix_min_required_version,
    is_forced:                    d.is_forced,
  });

  console.log('  ✓ App version updated (APK files must be re-uploaded manually in local admin)');
  console.log(`    serwise: ${d.serwise_version} (min: ${d.serwise_min_required_version})`);
  console.log(`    radix:   ${d.radix_version}   (min: ${d.radix_min_required_version})`);
  console.log(`    forced:  ${d.is_forced}`);
}

// ─── main ────────────────────────────────────────────────────────────────────

async function main() {
  console.log('🚀 Starting migration: prod → local\n');
  console.log(`   Source (prod):  ${SRC}`);
  console.log(`   Dest   (local): ${DST}`);

  const partMap = await migrateParts();
  await migrateSubscriptions(partMap);
  await migrateAppVersion();

  console.log('\n✅ Migration complete!');
  console.log('   ⚠️  Remember to re-upload APK files in the local Strapi admin → App Version.');
}

main().catch(e => { console.error('❌', e); process.exit(1); });
